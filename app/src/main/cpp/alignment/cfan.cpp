...
void CCFAN::GetSubImg(const unsigned char *gray_im, int im_width, int im_height, float point_x, float point_y, int patch_size, BYTE *sub_img)
{
  ...
  int lx = fabs(patch_left - ((center_x + 1) - patch_size / 2));
  int rx = patch_size - fabs(patch_right - ((center_x + 1) + patch_size / 2 - 1)) - 1;
  int ty = fabs(patch_top - ((center_y + 1) - patch_size / 2));
  int by = patch_size - fabs(patch_bottom - ((center_y + 1) + patch_size / 2 - 1)) - 1;
